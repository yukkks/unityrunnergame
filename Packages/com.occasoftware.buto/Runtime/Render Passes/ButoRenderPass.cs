...
namespace OccaSoftware.Buto.Runtime
{
    class RenderFogPass : ScriptableRenderPass
    {
        ...
        private const string profilerTag = "[Buto] Volumetic Fog";
        ...
        internal void SetupMaterials(Camera camera)
        {
            ...
            Vector3Int volumeSize = volumetricFog.GetVolumeSize(new Vector2Int(camera.pixelWidth, camera.pixelHeight));
            Vector3Int storedVolumeSize = Vector3Int.zero;

            if (kvps.TryGetValue(camera, out RTData rt))
            {
            ...
            if (rt.refreshTextures)
            {
                ...
                rt.lightingDataRtPrev.name = "LightPrevRT_" + UnityEngine.Random.Range(0, 1000);
            ...
            if (rt.refreshTextures)
            {
                ...
                rt.mediaDataRtPrev.name = "MediaPrevRT_" + UnityEngine.Random.Range(0, 1000);
        ...
        private void ExecutePass(PassData data, UnsafeGraphContext context)
        {
            ...
            ExecutePass(cmd, data.Camera, data.Source);
        }
#endif
        ...
        public RenderTextureDescriptor ConfigurePass(RenderTextureDescriptor cameraTextureDescriptor, Camera camera)
        {
            ...
            RenderTextureDescriptor volumeDescriptor = GetVolumeDescriptor(
              kvps[camera].volumeSize
            );

            RenderingUtilsHelper.ReAllocateIfNeeded(ref mediaTarget, volumeDescriptor);
            RenderingUtilsHelper.ReAllocateIfNeeded(ref lightingTarget, volumeDescriptor);
            RenderingUtilsHelper.ReAllocateIfNeeded(ref integratorTarget, volumeDescriptor);
            return rtDescriptor;
        }
        ...
        public override void Execute(ScriptableRenderContext context, ref RenderingData renderingData)
        {
            ...
            ExecutePass(cmd, renderingData.cameraData.camera, renderingData.cameraData.renderer.cameraColorTargetHandle);
        ...
        public void ExecutePass(CommandBuffer cmd, Camera camera, RTHandle source)
        {
        ...
        private void SetupMaterialData(CommandBuffer cmd, Camera cam)
        {
            ...
            cmd.SetGlobalFloat("buto_RealtimeSinceStartup", Time.realtimeSinceStartup);
